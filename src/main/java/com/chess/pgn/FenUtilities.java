...
public class FenUtilities {
    ...
    private static String calculateEnPassantSquare(final Board board) {
        ...
        if (enPassantPawn != null) {
            return BoardUtils.INSTANCE.getPositionAtCoordinate(enPassantPawn.getPiecePosition() + (8 * enPassantPawn.getPieceAlliance().getOppositeDirection())); // square behind enpassant piece
        }
        return "-";
    }
    ...
    private static String calculateCurrentPlayerText(final Board board) {
        return board.currentPlayer().toString().substring(0, 1).toLowerCase(); // b or w
    }
}
